...
export default function BackgroundSpotlight() {
    ...
    useEffect(() => {
        // Set hydration flag and initial media query values after mount
    ...
    useEffect(() => {
        ...
        const moveX = gsap.quickTo(spotlight, 'left', {
            duration: 1.2,
            ease: 'power2.out',
        });
        const moveY = gsap.quickTo(spotlight, 'top', {
            duration: 1.2,
            ease: 'power2.out',
        });
    ...
    if (!isHydrated || !isDesktop || prefersReducedMotion) {
        return null;
    }
    ...
    return (
        <div
            ...
            style={{
                background:
                    'radial-gradient(circle, var(--color-racing-red) 0%, transparent 70%)',
                filter: 'blur(120px)',
